jQuery(document).ready(function($){
	...
	dashboard.on('click', 'a.ajaxLink', function(event){
		...
		if( !target.hasClass('selected') && !isAnimating ) {
			...
			triggerAnimation(sectionTarget, sectionId, true);
		}
	  	...
	  	if( firstLoad ) {
		    ...
		    */
	        ...
	        newPage = newPageArray[newPageArray.length - 1].replace('', '');
	      	if( !isAnimating ) {
				triggerAnimation(newPage, 'home', false);
			}
	    }
	    firstLoad = true;
	});
	...
	//start animation
	function triggerAnimation(newSection, sectionId, bool) {
		isAnimating =  true;
		...
		initializeLoadingBar(sectionId);
		//load new content
		loadNewContent(newSection, sectionId, bool);
	}

	function initializeLoadingBar(section) {
		var	selectedItem =  dashboard.find('.selected'),
			windowHeight = $(window).height(),
			scaleValue = (windowHeight).toFixed(3)/1 + 0.001;
